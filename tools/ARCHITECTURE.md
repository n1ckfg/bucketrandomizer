...
1. **Formatting Fix (Normalization):** Sanitizes the ODT file by converting it back and forth through LibreOffice and manually fixing newline anomalies in the internal XML.
...
1. **LibreOffice Headless Conversion:** 
   - The script locates the LibreOffice binary (`soffice`).
   - It converts the input `.odt` into a `.docx` file in a temporary directory.
   - It then converts the `.docx` file back into an `.odt` file. This round-trip normalizes complex ODT-specific quirks and cleans up the document tree.
...
- The script uses a counter (`new_line_counter`) to accumulate empty lines. When it exceeds a threshold (`new_line_counter_max = 2`), it finalizes the current section and appends it to the extraction list.
...
```
1. Reads `input.odt`.
2. Creates `input_fixed.odt`.
3. Extracts content from `input_fixed.odt` into `sample.json`.
